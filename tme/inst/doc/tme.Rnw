...
(\cite{beswick77}). It is clear that this ideal situation is not often
...
\textrm{suvject to } Nv <= L, \textrm{($N$ an integer)}
\end{equation}
...
\cite{beswick77} use a response function of the following form:
...
z_i = 0.3258 (w_i/\overline{w})^0.172 (c_i/\overline{c}^0.646 p_i^0.694
(m_i/\overline{m})^0.105
\end{equation}
$$C_i = 0$$
$$a = 0.217$$
...
effectiveness of managÌng sales calls increase their
...
salesman or salesmanagment in allocating sales call time more efficiently.
The aim is to maximize the returns from their calls.
The model is based on the assumption that the expected sales to each
client is a function of the average number of calls per effort period
